...
async function handler(m, { conn, args, usedPrefix, command }) {
    ...
    let lol = `Lakukan format berikut ${usedPrefix}${command} [type] [value] [number]
Contoh ${usedPrefix}${command} money 9999 @tag
...
`.trim()
    ...
    if (!(who in global.db.data.users)) return m.reply(`User ${who} tidak tersedia didalam database.`)
...
handler.before = async m => {
    ...
    if (/y(es)?/g.test(m.text.toLowerCase())) {
        ...
        user[type] -= count * 1
        _user[type] += count * 1
        if (previous > user[type] * 1 && _previous < _user[type] * 1) m.reply(`Success transfer *${count}* ${rpg.emoticon(type)}${type}${special(type)} to *@${(to || '').replace(/@s\.whatsapp\.net/g, '')}*`, null, { mentions: [to] })
        else {
            user[type] = previous
